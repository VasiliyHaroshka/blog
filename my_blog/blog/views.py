...
def post_detail(request, year, month, day, post):
    ...
    recommend_posts = similar_posts.annotate(same_tags=Count("tags")).order_by("-same_tags", "-published_date")[:2]
...
def post_search(request):
    ...
    if form.is_valid():
        ...
        results = Post.published.annotate(search=SearchVector("title", "text")).filter(search=query)
    context = {
        "form": form,
        "results": results,
        "query": query
